...
impl FluidSimulation {
    ...
    pub fn divergence_step(&mut self) {
        for i in self.access_mask.index_range() {
            let neighbours = Self::NEIGHBOURS
                .iter()
                .map(|neighbour| self.access_mask.get(&self.is_wall, self.access_mask.index_to_pos(i) + neighbour, true))
            ...
            divergence -= vel.x;
            divergence -= vel.y;
            divergence += self.access_mask.get(&self.velocities, self.access_mask.index_to_pos(i) + Vector2::new(1, 0), Vector2::new(0.0, 0.0)).x;
            divergence += self.access_mask.get(&self.velocities, self.access_mask.index_to_pos(i) + Vector2::new(0, 1), Vector2::new(0.0, 0.0)).y;
            self.velocities[i] += Vector2::new(divergence / neighbours, divergence / neighbours);
            self.access_mask.set(&mut self.velocities, self.access_mask.index_to_pos(i) + Vector2::new(1, 0), |vel| {
                vel.x -= divergence / neighbours;
            });
            self.access_mask.set(&mut self.velocities, self.access_mask.index_to_pos(i) + Vector2::new(0, 1), |vel| {
    ...
    pub fn interpolate_vec(&self, grid: &[Vector2<f32>], pos: Vector2<f32>) -> Vector2<f32> {
        ...
        let weight = [
            (fx, fy), // Bottom left
            (1.0 - fx, fy), // Bottom right
            (fx, 1.0 - fy), // Top left
            (1.0 - fx, 1.0 - fy), // Top right
        ];

        const NEIGHBOURS: [Vector2<isize>; 4] = [
            Vector2::new(0, 0),  // Right
            Vector2::new(0, 1),  // Down
            Vector2::new(1, 0), // Left
            Vector2::new(1, 1), // Up
        ];*/
        self.access_mask.get(grid, pos, Vector2::new(0.0, 0.0))
    }

    pub fn interpolate(&self, grid: &[f32], pos: Vector2<f32>) -> f32 {
        let pos = Vector2::new(pos.x.floor() as isize, pos.y.floor() as isize);
        self.access_mask.get(grid, pos, 0.0)
    }
    ...
    pub fn advect(&mut self) {
        ...
        for i in self.access_mask.index_range() {
            ...
            let backtracked_vel = self.interpolate_vec(&old_velocities, float_pos - vel * self.time_step * self.cell_size);
            let backtracked_density = self.interpolate(&old_densities, float_pos - vel * self.time_step);
    ...
    pub fn tick(&mut self) {
        self.access_mask.set(&mut self.velocities, Vector2::new(0, 0), |vel| {
            *vel = Vector2::new(4.0, 0.0); // Reset velocities at the origin
        });
        self.access_mask.set(&mut self.densities, Vector2::new(0, 0), |density| {
